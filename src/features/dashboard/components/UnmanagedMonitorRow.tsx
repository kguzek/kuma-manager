...
export function UnmanagedMonitorRow({
  instance,
  monitor,
  suggestedTag,
  customTag,
    ...
    <div className="flex flex-col justify-between gap-3 rounded-2xl border p-4 md:flex-row md:items-center">
      <div>
        ...
        <div className="text-sm text-muted-foreground">
          {instance.config.name} · {monitor.url ?? monitor.hostname ?? "No URL/hostname"}
        </div>
      </div>
      <div className="flex min-w-80 flex-wrap items-center justify-end gap-2">
        {editing ? (
          <InputGroup className="w-72 font-mono">
            ...
            <InputGroupInput
              value={getTagSuffix(customTag ?? suggestedTag)}
              placeholder="example.com"
              onChange={(event) => onCustomTagChange(`monitor:${event.target.value.replace(/^monitor:/, "")}`)}
            />
          </InputGroup>
        ) : (
        ...
        <Button size="sm" variant="ghost" aria-label="Edit tag" onClick={onToggleEdit}>
          {editing ? <X /> : <Pencil />}
        </Button>
        <Button size="sm" variant="outline" disabled={!pendingTag.startsWith("monitor:")} onClick={() => onApplyTag(pendingTag)}>
          <Tags /> Add
        </Button>
      </div>
    </div>
