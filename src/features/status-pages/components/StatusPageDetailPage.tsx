...
import { ArrowLeft, Braces, FileBarChart, Save, Trash2, X } from "lucide-react"
...
import type { AppRoute, ConnectedKumaInstance, StatusPageDetailsValues, StatusPageSyncRecord } from "@/types"
...
export function StatusPageDetailPage({
  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {KNOWN_TEMPLATE_TOKENS.map((token) => {
                      const values = connectedInstances.map((inst) => token.resolve(inst.config.name))
      ...
      <PublicGroupsSection record={record} connectedInstances={connectedInstances} slug={slug} onNavigate={onNavigate} />
    </div>
  )
}
...
function PublicGroupsSection({
  ...
  const firstPage = connectedInstances.map((instance) => record.pagesByInstance[instance.config.id]).find(Boolean)
  const groups = firstPage?.publicGroupList ?? []
  ...
  return (
    <Card className="mt-4">
      <CardHeader>
        ...
        <CardDescription>
          Monitor groups displayed on the status page. Assign monitors to groups from the{" "}
          <RouteLink href="/monitors" onNavigate={onNavigate} className="underline">
            monitors page
          </RouteLink>
          .
        </CardDescription>
      ...
      <CardContent className="grid gap-3">
        {groups.length === 0 && <p className="text-sm text-muted-foreground">No monitor groups configured.</p>}
        {groups.map((group) => (
